...
export class DoctoraliaScraper {
    ...
    private async scrapeSearchPage(domain: string, city: string, specialty: string): Promise<ScrapedDoctor[]> {
        ...
        try {
            ...
            await page.goto(searchUrl, { waitUntil: 'networkidle2', timeout: this.env.SCRAPING_TIMEOUT_MS });
    ...
    private async scrapeDoctorProfile(url: string, city: string, specialty: string): Promise<ScrapedDoctor | null> {
        const page = await this.browser!.newPage();
        try {
            await page.goto(url, { waitUntil: 'domcontentloaded', timeout: this.env.SCRAPING_TIMEOUT_MS });

            // Extract basic info
            const fullName = await page.$eval('h1', el => el.textContent?.trim() || '').catch(() => 'Unknown');
            const address = await page.$eval('[data-testid="address-link"]', el => el.textContent?.trim() || '').catch(() => city); // Fallback

            // Rating
            const rating = await page.$eval('[data-testid="doctor-star-rating"]', el => {
                const text = el.textContent?.trim();
                return text ? parseFloat(text.replace(',', '.')) : 0;
            }).catch(() => 0);

            const reviewCount = await page.$eval('[data-testid="reviews-count"]', el => {
            ...
            const treatments = await page.$$eval('[data-testid="service-price-list-item"]', items =>
                items.slice(0, 5).map(item => {
                    ...
                    const priceText = item.querySelector('[data-testid="service-price"]')?.textContent?.trim();
                    // Parse price roughly
